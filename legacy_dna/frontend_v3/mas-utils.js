  ...
  const stableStringify = (obj) => {
    ...
    const f = x => {
      if (x && typeof x === 'object') {
        ...
        seen.add(x);
        // التعامل مع المصفوفات: تحويل undefined إلى null
        if (Array.isArray(x)) return '[' + x.map(v => v === undefined ? 'null' : f(v)).join(',') + ']';
        // التعامل مع الأوبجكت: حذف الخواص التي قيمتها undefined
        const keys = Object.keys(x).sort().filter(k => x[k] !== undefined);
        return '{' + keys.map(k => JSON.stringify(k) + ':' + f(x[k])).join(',') + '}';
      }
...
(function (w) {
  ...
  function fetchDefaultLibrary(basePath) {
    ...
    // User requested removal of env.css.default fetching.
    // We strictly use the active library or default 'tw' without network requests.
    cachedDefaultLibrary = activeLibrary || 'tw';
    return Promise.resolve(cachedDefaultLibrary);
  }
  ...
  const DEFAULT_PALETTE = {
    ...
    dark: {
      ...
      muted: 'hsl(220 28% 28%)',
      'muted-foreground': 'hsl(216 20% 82%)',
...
`;
  ...
  function isThemeInjectionEnabled() {
    const w = typeof window !== 'undefined' ? window : null;
    if (w && typeof w.MishkahThemeInjectionEnabled === 'boolean') return w.MishkahThemeInjectionEnabled;
    if (w && w.Mishkah && w.Mishkah.utils && typeof w.Mishkah.utils.themeInjectionEnabled === 'boolean') return w.Mishkah.utils.themeInjectionEnabled;
    return true;
  }
  function injectTheme(light, dark) {
    if (!isThemeInjectionEnabled()) return;
    ensureStyle('twcss-theme').textContent = varBlock(':root', light) + '\n' + varBlock(':root.dark', dark);
  }
  ...
  function auto(db, app, opt = {}) {
    ...
    if (useScaffold) {
      const s = scaffold({
        title: (db.head && db.head.title) || 'MAS App',
        fonts: opt.fonts,
  ...
  U.twcss = {
    ...
    setTheme,
    setDir,
    setThemeInjectionEnabled: (val) => { U.themeInjectionEnabled = !!val; },
    isThemeInjectionEnabled,
    setLibrary,
    getLibrary,
  ...
  function normalizePwaConfig(db, envConfig, runtimeOpt) {
    ...
    const inferredName = (manifestObj && manifestObj.name) || head.title || 'MAS App';
